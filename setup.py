    ...
    def run(self):
        if sys.platform == "darwin":
            ...
            try:
                ...
                print("Homebrew not found. Please install Homebrew to automatically install libomp.")
            
            if homebrew_installed:
                try:
                    subprocess.check_call(["brew", "install", "libomp"])
                except Exception as e:
                    print("Warning: Failed to install libomp using Homebrew. Please ensure libomp is installed.", e)
