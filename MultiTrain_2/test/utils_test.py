...
@pytest.mark.parametrize("custom_metric, expected_exception", [
    (None, None),
    ('invalid_metric', MultiTrainMetricError)
])
...
def test_non_auto_cat_encode_error(sample_dataframe):
    with pytest.raises(MultiTrainEncodingError):
        _non_auto_cat_encode_error(sample_dataframe, auto_cat_encode=False, manual_encode=None)
...
def test_handle_missing_values(sample_dataframe):
    ...
    fixed_df = _handle_missing_values(sample_dataframe, fix_nan_custom={'num_col': 'ffill'})
...
def test_fit_pred(models):
    ...
    model, prediction, time_taken = _fit_pred(models['LogisticRegression'], ['LogisticRegression'], 0, X_train, y_train, X_test)
